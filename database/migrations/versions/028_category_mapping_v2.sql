...
CREATE TABLE IF NOT EXISTS `category_platform_mappings` (
    ...
    `platform_category_id`        VARCHAR(100) NOT NULL     COMMENT '平台分类主ID（WB=subjectID, Ozon=description_category_id）',
    `platform_category_extra_id`  VARCHAR(100) DEFAULT NULL COMMENT '平台分类附加ID（Ozon=type_id，WB 为空）',
