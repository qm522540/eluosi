...
### 规则 6：时间统一规范 — DB 存 UTC / 判断用 MSK / Celery 走 pytz

服务器 OS 曾是 CST，MySQL time_zone=SYSTEM，导致 `NOW()` 返回北京时间当 UTC 存，
DB 里时间字段整体错位 8 小时。2026-04-23 大改造后统一规范如下。

**存储**：所有 DATETIME 字段一律真 UTC naive。
- **SQL 禁用**：`NOW()` / `CURRENT_TIMESTAMP` / `DATE_ADD(NOW(), ...)` 作为 INSERT/UPDATE 值
- **Python 禁用**：`datetime.now()`（无 tz）/ `datetime.utcnow()`（3.12 弃用）
- **唯一写入入口**：`utc_now_naive()`（from `app.utils.moscow_time`）— 返回 `datetime.now(timezone.utc).replace(tzinfo=None)`
- 写法：
  ```python
  ...
  ```
- 表定义的 `DEFAULT CURRENT_TIMESTAMP` / `ON UPDATE CURRENT_TIMESTAMP` 可以保留（迁移历史），
  但**不依赖其值**——运行时所有时间字段都显式传 `:now_utc`

**业务判断**：一律用莫斯科时间工具（from `app.utils.moscow_time`）
- `now_moscow()` → aware MSK datetime
- `moscow_hour()` → 0-23
- `moscow_today()` → 今天 MSK 日期（判"次日过期"）
- 判"是不是凌晨低谷"/"今天"/"跨日切换"都要走这三个函数

**展示**：naive UTC datetime → MSK ISO 串用 `moscow_time._iso()`

**Celery beat**：保持 `timezone="Europe/Moscow"` 配置，crontab 小时直解为 MSK，
不受 OS 时区影响（pytz 基于真 UTC 计算）。参考 memory `reference_celery_beat_timezone.md`。

**自查**：
```bash
grep -rn "NOW()\\|CURRENT_TIMESTAMP" app/services/ app/api/ | grep -v "CREATE TABLE\\|ALTER TABLE\\|#"
grep -rn "datetime\\.now()\\|datetime\\.utcnow" app/
```
业务 SQL 里出现 NOW() 视为违规（除了迁移文件 CREATE/ALTER）。
