...
_ASSIGN_PROMPT = """你是俄语电商关键词聚类专家。任务：把给定的关键词列表分配到已指定的固定集群里。

规则：
1. 每个关键词必须分到**恰好一个**集群（按语义最相近）
2. 若某个关键词和所有集群都不搭，放入 "_other"
3. 同义/形态变化合并：серьги↔серёжки、детские↔для детей 归同簇
4. 修饰词不影响归属：加了 большие/маленькие/цветные/10 лет 等仍归入主簇
5. 核心属性决定归簇：含 "медицинский сплав" 归"серьги детские медицинский сплав"；含 "для девочек"/"для девочки"/"маленьких девочек" 归"серьги для девочек"（如果两个都有以最显著的核心属性为准）

输出严格 JSON（不要 markdown）：
{"assignments": {"集群名1": ["关键词1", "关键词2", ...], "集群名2": [...], "_other": [...]}}
每个输入关键词必须出现在**某一个**桶里。
"""
