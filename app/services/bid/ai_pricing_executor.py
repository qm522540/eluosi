...
def _current_period_hint() -> str:
    """生成当前莫斯科时段的 prompt 注入片段（仅作次要参考权重，ROAS 仍为主轴）"""
    h = moscow_hour()
    if h in (19, 20, 21):
        return (f"【当前时段】莫斯科 {h}:00，处于【高峰期】"
                f"— 流量最大竞争激烈：ROAS 接近目标的商品可倾向加价抢量（±5%参考权重）")
    if h in (10, 11, 12, 22):
        return (f"【当前时段】莫斯科 {h}:00，处于【次高峰】"
                f"— 流量较大：维持中性策略")
    if 1 <= h <= 7:
        return (f"【当前时段】莫斯科 {h}:00，处于【低谷期】"
                f"— 流量最少：ROAS 接近目标的商品可倾向降价省预算（±5%参考权重）")
    return (f"【当前时段】莫斯科 {h}:00，处于【平谷期】"
            f"— 正常流量：维持中性策略")
...
async def _analyze_now_inner(db, tenant_id: int, shop_id: int,
    ...
    for raw in suggestions_raw:
        ...
        # 硬性安全护栏（风险不对称）：
        #   days<7:     全丢（数据太少一切都不信）
        #   days 7-9:   只允许降价建议（及早止损，风险小）, 加价/维持丢弃
        #   days>=10:   正常决策, 但 trend=down 禁止加价
        #   trend=new:  全丢（连趋势都算不出来）
        sku_stat = sku_stats.get(f"{camp_id}_{sku}", {})
        days = int(sku_stat.get("days", 0) or 0)
        trend = sku_stat.get("trend", "new")
        if days < 7:
            logger.warning(f"[guardrail] 丢弃 sku={sku} days={days}<7 (AI建议{current_bid}->{suggested_bid})")
            continue
        if trend == "new":
            logger.warning(f"[guardrail] 丢弃 sku={sku} trend=new (AI建议{current_bid}->{suggested_bid})")
            continue
        if days < 10 and suggested_bid >= current_bid:
            logger.warning(f"[guardrail] 丢弃 sku={sku} days={days}<10 仅允许降价 (AI建议{current_bid}->{suggested_bid})")
            continue
...
==========================================
规则A：total_10d.days < 7 的商品，**禁止给任何调价建议**（数据太少一切都不信）。
规则B：total_10d.days 在 7-9 之间（即 7/8/9），**只允许给降价建议**（suggested_bid < current_bid）。
       加价或维持出价的建议会被丢弃。理由：数据不够时加价风险大, 但及早止损是保护动作。
规则C：trend="down" 的商品，suggested_bid **不得高于** current_bid（禁止加价，只能维持或降价）。
规则D：trend="new" 的商品，**禁止给任何调价建议**。
违反以上任何一条的建议，会被代码层 100% 过滤掉，纯属浪费 token，请严格遵守。
==========================================

**核心目标：ROAS 最大化**（不是达到目标就好，而是让 ROAS 尽可能大）

决策逻辑（按优先级，所有规则都要先满足上面四条硬规则）：

1. **数据不足**（days < 7 或 trend="new"）→ 不要返回此商品（见硬规则A/D）
2. **ROAS >= 目标ROAS × 2 + 数据 >= 10天 + 趋势稳定或上升** → product_stage="growing"，小幅加价5-10%
3. **ROAS >= 目标ROAS × 2 + 数据 >= 10天 + 趋势下降** → 不要返回（见硬规则C，趋势下降禁加价）
4. **ROAS >= 目标ROAS + 数据 >= 10天 + 趋势稳定或上升** → product_stage="growing"，小幅加价5-10%
5. **ROAS >= 目标ROAS + 数据 >= 7天 + 趋势下降** → product_stage="testing"，小幅降价5-10%（降价可在 days>=7 时执行）
6. **最低ROAS <= ROAS < 目标ROAS + 数据 >= 7天** → product_stage="testing"，小幅降价5-10%
7. **ROAS < 最低ROAS + 数据 >= 7天 + 趋势下降或持平** → product_stage="declining"，大幅降价到当前出价的70%
8. **ROAS < 最低ROAS + 数据 >= 10天 + 趋势上升** → product_stage="testing"，不动，正在恢复
9. **有点击无订单（cr=0）+ 花费持续 + 数据 >= 7天** → product_stage="declining"，降价
...
【流量时段规律（莫斯科时间）】
- 高峰期 19:00-21:00（流量最大，竞争激烈，CPC高）
- 次高峰 10:00-12:00, 22:00（流量较大）
- 低谷期 01:00-07:00（流量最少）
- 平谷期 其余时间（正常流量）

{_current_period_hint()}

**时段影响规则**：时段作为次要参考权重（约±5%倾向），主决策轴仍是 ROAS 和 trend。
仅当某商品的 ROAS 很接近目标ROAS（差异<20%）、决策方向不明确时，才使用时段权重微调方向。
ROAS 明确需要加价/降价的商品，完全以 ROAS 决策为准，不受时段影响。
{history_section}
【活动和商品数据（出价单位：卢布）】
