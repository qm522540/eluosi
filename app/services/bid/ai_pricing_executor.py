...
def _calc_healthy_window_metrics(db, tenant_id: int, campaign_id: int, sku: str,
                                  recent_n: int, skip_n: int = 0,
                                  platform: str | None = None) -> tuple:
    """剔除噪声日后，取从近到远第 skip_n+1 ~ skip_n+recent_n 个健康天的聚合数据。
    噪声定义（双条件 AND）：当天 ROAS > 50 AND spend < ₽5（极小投入偶然爆单）。
    单纯 ROAS 高不算噪声（长尾 SKU 投 ₽7 出 1 单 ROAS 67x 是真信号）。
    单纯小投入也不算噪声（长尾 SKU 每天 ₽5-10 投入是常态）。
...
def _calc_healthy_window_full(db, tenant_id: int, campaign_id: int, sku: str,
    ...
    噪声判定同前函数：ROAS > 50 AND spend < ₽5 才剔除。
...
def _hill_climbing_decision(db, tenant_id: int, campaign_id: int, sku: str,
    ...
    rw = _calc_healthy_window_full(db, tenant_id, campaign_id, sku, 5, 0, platform)
    if rw["impressions"] > 0:
    ...
    if is_cold_start:
        ...
        sp_l5, rv_l5, d_l5    = _calc_healthy_window_metrics(db, tenant_id, campaign_id, sku, 5, 0, platform=platform)
        sp_p5, rv_p5, d_p5    = _calc_healthy_window_metrics(db, tenant_id, campaign_id, sku, 5, 5, platform=platform)
        sp_all, rv_all, d_all = _calc_healthy_window_metrics(db, tenant_id, campaign_id, sku, 28, 0, platform=platform)
        sku_stat_clean = {
            **sku_stat,
...
async def _analyze_now_inner(db, tenant_id: int, shop_id: int,
    ...
    # WB 拉商品过程中若触发 seller 级 quota 熔断（fetch_campaign_products 内部吞了
    # WBSellerQuotaExhausted 返回 []，但 trip 已写 Redis key），这里二次 check 给前端
    # 准确文案，避免误显示"分析完成 0 条建议"让用户以为正常
    if platform == "wb":
        try:
            cooldown_now = client._check_quota_circuit()
        except Exception:
            cooldown_now = None
        if cooldown_now:
            await client.close()
                ...
                if is_losing:
                    ...
                    if (cfg.auto_remove_losing_sku and cfg.auto_execute
                        ...
                        rw = _calc_healthy_window_full(
                            db, tenant_id, camp.id, sku, 5, 0, platform=platform)
                        if rw["impressions"] > 0:
                            r_ctr = rw["clicks"]  / rw["impressions"]
                            r_cr  = rw["orders"]  / rw["clicks"] if rw["clicks"] > 0 else 0
...
def _get_roas_21_30(db, campaign_id: int, sku: str,
    ...
    新规则：从 today-21 往前数，跳过噪声日（双条件 AND：ROAS>50 AND spend<₽5），
