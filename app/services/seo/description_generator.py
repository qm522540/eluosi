...
4. 关键词自然融入正文，**禁止罗列堆砌**。绝对禁止：
   - **不要用 # 开头的 hashtag** (如 `#серьгиженские #бижутерия` — Ozon 审核会直接拒)
   - 不要写 "Ключевые слова: ..." / "Теги: ..." / "Поисковые запросы: ..." 类罗列段
   - 不要在末尾追加任何关键词云 / 标签段 / 站内搜索词清单
   - 关键词必须**自然融入正文句子**,不是单独成段也不是井号标记
   违反任何一条 = 此次生成失败
...
8. **全俄语，禁止中英混词** —— 不要写出 `котороеperfectо` 这种俄文里突然插英文的怪词。
   英文品牌名 (Pt.Girl / Shariko) / 国际通用词 (happy birthday) 可整词保留,
   但**不能跟俄文字母粘连**形成新单词。
9. 标点正常使用（描述不像标题那样无标点，正常的逗号句号必须有，让用户读得舒服）。
   **注意 JSON 转义**: 如果描述里要用引号 (如 «Леопард» 商品名),用俄文书名号 `«»`
   或单引号 `'`,**绝对禁止**用 ASCII 双引号 `"..."`,会让你输出的 JSON 解析失败。
...
# ==================== AI 输出后处理 (兜底,SYSTEM_PROMPT 失守时) ====================

# 行级匹配: 整行只有 hashtag / 整行像"Ключевые слова: xxx"/"Теги: xxx"罗列
_HASHTAG_LINE_RE = re.compile(r"^\s*(?:#\S+\s*)+$", re.MULTILINE)
_KW_DUMP_LINE_RE = re.compile(
    r"^\s*(?:ключевые\s+слова|теги|tags|поисковые\s+запросы|поисковые\s+слова|search\s+terms)"
    r"\s*[:：].*$",
    re.IGNORECASE | re.MULTILINE,
)
# inline hashtag (句中残留的 #word)
_HASHTAG_INLINE_RE = re.compile(r"#\S+")


def _strip_keyword_dump(s: str) -> tuple[str, dict]:
    """清掉 AI 在描述末尾追加的 hashtag 块 / "Ключевые слова: ..." 罗列段。

    Ozon 审核对 # 标签和"标签罗列段"会拒审,必须兜底剥离。
    返 (cleaned, removed) — removed 用于审计 log。
    """
    if not s:
        return s, {}
    removed = {}
    cleaned = s
    # 1. 整行 hashtag 块 → 删行
    htags = _HASHTAG_LINE_RE.findall(cleaned)
    if htags:
        removed["hashtag_lines"] = htags
        cleaned = _HASHTAG_LINE_RE.sub("", cleaned)
    # 2. "Ключевые слова: xxx" 类罗列段 → 删行
    kw_dumps = _KW_DUMP_LINE_RE.findall(cleaned)
    if kw_dumps:
        removed["kw_dump_lines"] = kw_dumps
        cleaned = _KW_DUMP_LINE_RE.sub("", cleaned)
    # 3. 句中残留的 #word → 删除 (但保留正常文字)
    inline = _HASHTAG_INLINE_RE.findall(cleaned)
    if inline:
        removed["inline_hashtags"] = inline
        cleaned = _HASHTAG_INLINE_RE.sub("", cleaned)
    # 4. 收尾: 多余空行合并 + 末尾 strip
    cleaned = re.sub(r"\n{3,}", "\n\n", cleaned).strip()
...
OZON_ATTR_BLACKLIST = {
    ...
    9661,   # 包装数量 (Количество в упаковке)
    8513,   # 包装数量,件 (Количество в упаковке, шт)
    11650,  # 工厂包装数量 (Количество заводских упаковок)
}
...
def _parse_ai_output(raw: str) -> dict:
    ...
    # Fallback 1: AI 经常在 description 里写 "Леопард"/"Premium" 这种内部双引号没转义,
    # 整个 JSON 解析挂掉。用正则直接抽 4 个字段(greedy + DOTALL),容忍内部未转义引号。
    desc_m = re.search(
        r'"new_description"\s*:\s*"(.*)"\s*,\s*"reasoning"',
        raw, re.DOTALL,
    )
    if desc_m:
        new_desc = desc_m.group(1).strip()
        reason_m = re.search(
            r'"reasoning"\s*:\s*"(.*?)"\s*,\s*"included_keywords"',
            raw, re.DOTALL,
        )
        kws_m = re.search(
            r'"included_keywords"\s*:\s*\[(.*?)\]',
            raw, re.DOTALL,
        )
        struct_m = re.search(
            r'"structure"\s*:\s*\[(.*?)\]',
        ...
        # 解 list 里的字符串 (re 简陋抽取)
        def _parse_list_strs(s: str) -> list:
            if not s:
                return []
        ...
        return {
            ...
            "reasoning": (reason_m.group(1).strip() if reason_m else
                          "（AI JSON 内部双引号未转义，已用正则 fallback 抽取字段）"),
            "included_keywords": _parse_list_strs(kws_m.group(1) if kws_m else ""),
            "structure": _parse_list_strs(struct_m.group(1) if struct_m else ""),
        }

    # Fallback 2: 连正则都 match 不到 → 整段当描述（最后兜底）
...
async def generate_description(
    ...
    # post-check: 剥离 # hashtag / "Ключевые слова: xxx" 罗列段 (Ozon 审核会拒)
    raw_desc = parsed["new_description"]
    cleaned_desc, removed_blocks = _strip_keyword_dump(raw_desc)
    if removed_blocks:
        logger.warning(
            f"SEO description post-check: AI 输出含违禁块已被清洗 product={product_id} "
            f"removed={removed_blocks} 原长={len(raw_desc)} 净长={len(cleaned_desc)}"
        )
    parsed["new_description"] = cleaned_desc
